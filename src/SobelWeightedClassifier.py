...
def extract_sobel_features(image_path):
    """Extract Sobel edge detection features from an image"""
    ...
    # Calculate features
    sobel_edge_density = np.sum(sobel_combined > 0) / sobel_combined.size
    sobel_intensity = np.mean(sobel_combined)
    
    return {
        'sobel_edge_density': sobel_edge_density,
        'sobel_intensity': sobel_intensity
    }

def classify_image_sobel(features, threshold=0.71):
    """Classify image as Cracked or Non-cracked using weighted Sobel features"""
    if features is None:
        return None
    
    # Normalize features to [0, 1] range for balanced contribution
    # Assuming edge_density is already in [0, 1], normalize intensity
    edge_density_norm = min(features['sobel_edge_density'], 1.0)
    intensity_norm = min(features['sobel_intensity'] / 255.0, 1.0)
    
    # Weighted combination: edge density has higher weight as it's more indicative of cracks
    crack_score = (0.7 * edge_density_norm) + (0.3 * intensity_norm)
    
    # Classification based on combined score
    prediction = crack_score > threshold
...
for cat in categories:
    for label in labels:
        ...
        for img_path in image_paths:
            features = extract_sobel_features(img_path)
            if features is None:
                continue
            
            predicted_label = classify_image_sobel(features)
            actual_label = label
            ...
            results.append({
                ...
                'Correct': is_correct,
                'Sobel_Edge_Density': features['sobel_edge_density'],
                'Sobel_Intensity': features['sobel_intensity']
            })
...
axes[0].set_title("Sobel - Classification Accuracy per Category", fontsize=14, fontweight='bold')
...
# Save results to CSV
#df_results.to_csv("sobel_classification_results.csv", index=False)
#print("\nResults saved to 'sobel_classification_results.csv'")