...
def classify_image_canny(features, threshold=0.03):
    """Classify image as Cracked or Non-cracked using weighted Canny features"""
    if features is None:
        return None
    
    # Normalize features to [0, 1] range for balanced contribution
    edge_density_norm = min(features['canny_edge_density'], 1.0)
    intensity_norm = min(features['canny_intensity'] / 255.0, 1.0)
...
# features_to_plot = ['Canny_Edge_Density', 'Canny_Intensity']
# for idx, feature in enumerate(features_to_plot):
#     ax = axes[idx]
    
#     cracked_data = df_results[df_results['Actual_Label'] == 'Cracked'][feature]
#     non_cracked_data = df_results[df_results['Actual_Label'] == 'Non-cracked'][feature]
    
#     ax.hist(cracked_data, bins=20, alpha=0.6, label='Cracked', color='#FF5733')
#     ax.hist(non_cracked_data, bins=20, alpha=0.6, label='Non-cracked', color='#3357FF')
#     ax.set_xlabel(feature, fontsize=11)
#     ax.set_ylabel("Frequency", fontsize=11)
#     ax.set_title(f"Canny - Distribution of {feature}", fontsize=12, fontweight='bold')
#     ax.legend()
#     ax.grid(alpha=0.3)
